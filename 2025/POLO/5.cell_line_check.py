...
exp = pd.read_csv('/home/jiye/jiye/sillajen/data/CCLE/CCLE_expression.csv', sep=',',index_col=0)
exp = exp.loc[('ACH-000713','ACH-000811'),:]
exp.columns = exp.columns.str.replace(r"\s*\(\d+\)", "", regex=True)
exp.index = ['Caov-3','SK-OV-3']
# %%
SF_genes = [
...
SF_selected = ['SRSF7','SRSF9','SRSF2','THOC3','SNRNP25' ]
SF_selected = ['SNRNP25','SRSF9','UBL5','U2AF2','PTBP1','RBM42','YBX1','PRPF31']
exp_SF_selected = exp.loc[:,exp.columns.isin(SF_selected)]
exp_SF_full = exp.loc[:,exp.columns.isin(SF_genes)]

# %%
GDSC1 = pd.read_excel("/home/jiye/jiye/sillajen/data/CCLE/GDSC1_fitted_dose_response_27Oct23.xlsx")
GDSC2 = pd.read_excel("/home/jiye/jiye/sillajen/data/CCLE/GDSC2_fitted_dose_response_27Oct23.xlsx")

# %%
cell_lines = ['Caov-3','SK-OV-3']
drugs = ['Olaparib','Niraparib']

GDSC1_filtered = GDSC1.loc[(GDSC1['CELL_LINE_NAME'].isin(cell_lines)) & (GDSC1['DRUG_NAME'].isin(drugs)), ['CELL_LINE_NAME','DRUG_NAME','LN_IC50']]
GDSC2_filtered = GDSC2.loc[(GDSC2['CELL_LINE_NAME'].isin(cell_lines)) & (GDSC2['DRUG_NAME'].isin(drugs)), ['CELL_LINE_NAME','DRUG_NAME','LN_IC50']]
# %%
