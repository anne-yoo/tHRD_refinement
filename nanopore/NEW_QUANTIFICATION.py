    ...
    with open(stringtie_gtf, 'r') as infile:
        for line in infile:
            ...
            if len(fields) > 8:
                ...
                if gene_id.startswith("MSTRG") and (gene_name != "NA" ):
...
stringtie_gtf_path = "/home/jiye/jiye/copycomparison/GENCODEquant/POLO_hg38/merged_83_cov5.gtf"
...
filtered_output_path = "/home/jiye/jiye/copycomparison/GENCODEquant/POLO_hg38/merged_83_filtered_transcripts_with_gene_info.tsv"
...
def extract_tpm_for_target_transcripts(gtf_file, target_transcripts_set):
    ...
    with open(gtf_file, 'r') as file:
        for line in file:
            ...
            tpm_value = attr_dict.get("TPM") #*cov / TPM
...
def build_tpm_matrix_fast(base_directory, matrix_file_path):
    # Load the matrix file: mstrg_gene_id, transcript_id, gene_name
    matrix_df = pd.read_csv(matrix_file_path, sep="\t")

    # Filter to target transcripts
    target_transcripts_set = set(matrix_df['transcript_id'])

    # Build transcript_id → final_gene_label mapping
    def resolve_gene_label(row):
        return row['gene_name'] if pd.notnull(row['gene_name']) and row['gene_name'] != 'NA' else row['mstrg_gene_id']
    matrix_df['final_gene_label'] = matrix_df.apply(resolve_gene_label, axis=1)
    transcript_gene_map = dict(zip(matrix_df['transcript_id'], matrix_df['final_gene_label']))

    # Initialize a dictionary to store TPM values
    tpm_data = {}
    ...
    for sample_dir in os.listdir(base_directory):
        ...
        gtf_file = os.path.join(sample_path, f"{sample_dir}.gtf")  # assumes this naming convention
        ...
        if os.path.exists(gtf_file):
            ...
            for transcript_id, tpm_value in sample_tpm_values.items():
                gene_label = transcript_gene_map.get(transcript_id, 'NA')
                transcript_key = f"{transcript_id}-{gene_label}"
...
# Example usage
base_directory = "/home/jiye/jiye/copycomparison/GENCODEquant/SEV_pre/111_pre/quantforval"  # Update with the correct path
matrix_file_path = "/home/jiye/jiye/copycomparison/GENCODEquant/SEV_prepost/cov5_filtered_transcripts_with_gene_info.tsv"  # Update with the correct path
tpm_df = build_tpm_matrix_fast(base_directory, matrix_file_path)

# Save the DataFrame to a file
output_file_path = "/home/jiye/jiye/copycomparison/GENCODEquant/SEV_pre/111_pre/forval_111_transcript_TPM.txt"  # Update with the desired output path
tpm_df.to_csv(output_file_path, sep="\t")
print(f"TPM matrix saved to {output_file_path}")

#%%
###^^^^^^ gene level from transcript tpm ##############
import pandas as pd
import pandas as pd
import numpy as np
import re
...
# 입력 경로 설정
# -----------------------------
TPM_PATH = "/home/jiye/jiye/copycomparison/GENCODEquant/SEV_pre/111_pre/forval_111_transcript_TPM.txt"
...
transcript_tu = tpm.iloc[:, :-1] / gene_sum_aligned

# --- check NaN counts directly ---
nan_counts = transcript_tu.isna().sum(axis=1)
print("Rows containing NaN:", (nan_counts > 0).sum())

# --- save with visible NaN ---
# tu_out = TPM_PATH.replace("_transcript_TPM.txt", "_transcript_TU_withna.txt")
# transcript_tu.to_csv("/home/jiye/jiye/copycomparison/GENCODEquant/SEV_pre/2025new/34_transcript_TU_withna.txt", sep="\t", na_rep="NaN")
# transcript_tu.fillna(0).to_csv("/home/jiye/jiye/copycomparison/GENCODEquant/SEV_pre/2025new/34_transcript_TU.txt", sep="\t")
gene_tpm.to_csv("/home/jiye/jiye/copycomparison/GENCODEquant/SEV_pre/111_pre/forval_111_gene_TPM.txt", sep="\t")
tpm.to_csv('/home/jiye/jiye/copycomparison/GENCODEquant/SEV_pre/111_pre/forval_111_transcript_TPM.txt',sep='\t')
#print(f"✅ TU with real NaN saved to {tu_out}")


# transcript_tu.loc[transcript_tu.index.str.endswith("BRIP1")].sum(axis=0).head()
# transcript_tu.isna().apply(lambda row: list(row.index[row]), axis=1)
#%%
...
def create_gene_abundance_matrix(transcript_tpm_matrix_path, mapping_file_path, output_file_path):
    # Load the transcript TPM matrix (index = transcript_id-gene_name)
    tpm_df = pd.read_csv(transcript_tpm_matrix_path, sep="\t", index_col=0)

    # Extract transcript_id and gene_name_from_index
    tpm_df['transcript_id'] = tpm_df.index.to_series().str.split("-", n=1).str[0]
    tpm_df['gene_name_from_index'] = tpm_df.index.to_series().str.split("-", n=1).str[1]

    # Load mapping file (transcript_id ↔ gene_name / mstrg_gene_id)
    mapping_df = pd.read_csv(mapping_file_path, sep="\t")
    mapping_df = mapping_df.drop_duplicates(subset=["transcript_id"])  # 중복 방지

    tx2gene = mapping_df.set_index("transcript_id")["gene_name"].to_dict()
    tx2mstrg = mapping_df.set_index("transcript_id")["mstrg_gene_id"].to_dict()

    # Final gene label: gene_name if available else mstrg_gene_id
    def get_gene_label(tx):
        gene = tx2gene.get(tx)
        if pd.notnull(gene) and gene != "NA":
            return gene
        else:
            return tx2mstrg.get(tx)

    tpm_df["final_gene_label"] = tpm_df["transcript_id"].map(get_gene_label)

    # Group by final_gene_label and sum counts
    value_cols = [c for c in tpm_df.columns if c not in ["transcript_id", "gene_name_from_index", "final_gene_label"]]
    gene_tpm_df = tpm_df.groupby("final_gene_label")[value_cols].sum()

    # Save
    gene_tpm_df.to_csv(output_file_path, sep="\t")
    print(f"✅ Gene abundance matrix saved to {output_file_path}")


# Example usage
transcript_tpm_matrix_path = "/home/jiye/jiye/nanopore/2025finaldata/quantification/LR_289_transcript_coverage.txt"  # Update with the correct path
matrix_file_path = "/home/jiye/jiye/nanopore/2025finaldata/quantification/LR_filtered_transcripts_with_gene_info.tsv"  # Update with the correct path
output_file_path = "/home/jiye/jiye/nanopore/2025finaldata/quantification/LR_289_gene_coverage.txt"  # Update with the desired output path
create_gene_abundance_matrix(transcript_tpm_matrix_path, matrix_file_path, output_file_path)


#%%
##**** gene readcount from transcript readcount matrix ##################
import pandas as pd

# 1. transcript readcount matrix 불러오기
# 첫 컬럼이 transcript_id, 나머지가 샘플별 readcount
counts = pd.read_csv("/home/jiye/jiye/nanopore/2025finaldata/SRLR_comparison/SR_238_transcript_TPM.txt", sep='\t', index_col=0)

# 2. transcript → gene 매핑 불러오기
info = pd.read_csv("/home/jiye/jiye/nanopore/2025finaldata/SRLR_comparison/SR_filtered_transcripts_with_gene_info.tsv", sep="\t")

# 3. transcript_id → gene_name dict 만들기
tx2gene = dict(zip(info["transcript_id"], info["gene_name"]))

# 4. transcript_id 기준으로 counts 행 이름 매핑
counts["gene_name"] = counts.index.map(tx2gene)

# transcript_id가 매핑 안 되는 경우는 NaN 생김 → 필요시 drop
counts = counts.dropna(subset=["gene_name"])

# 5. gene_name 기준으로 sum
gene_counts = counts.groupby("gene_name").sum()

# 6. 저장
gene_counts.to_csv("/home/jiye/jiye/nanopore/2025finaldata/SRLR_comparison/SR_239_gene_TPM.txt", sep='\t')
...
# %%
tmp = pd.read_csv('/home/jiye/jiye/nanopore/HG_JH_check/mixSRLR/quantification/transcript_count_matrix.csv', sep=',', index_col=0)
tmp = tmp.fillna(0)
tmp.to_csv('/home/jiye/jiye/nanopore/HG_JH_check/mixSRLR/quantification/transcript_count_matrix_fillna.csv', sep=',', index=True)
# %%
