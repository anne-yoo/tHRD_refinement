...
def getGeneID(attr_field, ctg, tid):
    r = RE_GENE_ID.search(attr_field)
    rn = RE_GENE_NAME.search(attr_field)
    if r:
        return r.group(1) + '|' + rn.group(1) if rn else r.group(1)
    return tid

geneIDs = defaultdict(str)  # transcript_id -> gene_id
badGenes = []  # non-MSTRG genes to be clustered

print("🔄 Scanning all GTFs to populate geneIDs and badGenes...")

for sample_id, gtf_path in samples:
    try:
        with open(gtf_path) as f:
            split = [l.split('\t') for l in f if not l.startswith('#')]
        for i, v in enumerate(split):
            if len(v) != 9 or v[2] != "transcript":
                continue
            try:
                t_id = RE_TRANSCRIPT_ID.search(v[8]).group(1)
                g_id = getGeneID(v[8], v[0], t_id)
                geneIDs[t_id] = g_id
                if not re.match(opts.string, g_id):  # not MSTRG-style → cluster candidate
                    this_gene = [v[0], v[6], t_id, g_id, min(int(v[3]), int(v[4])), max(int(v[3]), int(v[4]))]
                    j = i + 1
                    while j < len(split) and split[j][2] == "exon":
                        this_gene.append((min(int(split[j][3]), int(split[j][4])), max(int(split[j][3]), int(split[j][4]))))
                        j += 1
                    badGenes.append(this_gene)
            except Exception as e:
                print(f"[WARNING] Failed to parse transcript in {sample_id}: {v}")
    except FileNotFoundError:
        print(f"[ERROR] GTF file not found for sample {sample_id}")
...
##        print i,v
       try:
            ...
            if s[0] in v:
                ...
                print(f"[Warning] Missing sample '{s[0]}' in transcript {i}, skipping.")
