...
#%%
##^ correlation: pre AI ~ post AI matched variant correlation by gene ids
wesplot = makeplotdf(0)
wgsplot = makeplotdf(1)
dflist = [wesplot,wgsplot]
filenamelist = ['WES samples', 'WGS samples']
...
for i in range(2):
    ...
    df['AI'] = df['RNA_AF'] - 0.5
    df = pd.merge(df,geneid,on='ENSG')

    ###&########
    df = df[df['geneid']=='RPA1']
    ###&########

    pre = df[df['pre/post']=='pre']
    post = df[df['pre/post']=='post']

    pre = pre[['sample','position','AI', 'geneid']]
    post = post[['sample','position','AI', 'geneid']]
    
    merged = pd.merge(pre,post,how='inner',on=['position','sample'])
    merged = merged[['sample','position','AI_x','AI_y','geneid_x']]
    merged.columns = ['sample','position','pre_AI','post_AI','geneid']

    print(merged.groupby('geneid')[['pre_AI','post_AI']].corr().unstack().iloc[:,1])
...
for i in range(2):
    ...
    condition1 = (0.4<=df['genome_AF']) & (df['genome_AF'] <= 0.6)
    df = df.loc[condition1,:]
    df = df[['sample','pre/post','deltaAF','geneid']]
    ...
    sns.catplot(x ='pre/post', y ='deltaAF', hue = 'geneid', data = df, kind='point', palette='tab10', order=['pre','post'], height = 4.5, aspect = 0.9)
    
    plt.suptitle('<'+filenamelist[i]+': pre vs. post delta AF>', y=1.03, size=11)
    plt.show()
...
for i in range(2):
    ...
    condition1 = (0.4<=df['genome_AF']) & (df['genome_AF'] <= 0.6)
    df = df.loc[condition1,:]
    df = df[['sample','pre/post','AI','geneid']]

    sns.set(rc = {'figure.figsize':(6,6)})
    sns.set_style('whitegrid')
    sns.catplot(x ='pre/post', y ='AI', hue = 'geneid', data = df, kind='point', palette='tab10', order=['pre','post'], height = 4.5, aspect = 0.9)
    plt.suptitle('<'+filenamelist[i]+': pre vs. post AI>', y=1.03, size=11)
    plt.show()
...
for i in range(2):
    ...
    merged = pd.merge(pre,post,how='inner',on=['position','sample'])
    m_pre = merged[['sample','position','AI_x', 'geneid_x']]
    m_post = merged[['sample','position','AI_y','geneid_y']]
    m_pre.columns = ['sample','position','AI','geneid']
    m_post.columns = ['sample','position','AI','geneid']
    m_pre['pre/post'] = 'pre'
    ...
    sns.set(rc = {'figure.figsize':(6,6)})
    sns.set_style('whitegrid')
    sns.catplot(x ='pre/post', y ='AI', hue = 'geneid',  data = concatdf, kind='point', palette='tab10', order=['pre','post'], height = 4.5, aspect = 0.9)
    # sns.barplot(x='sample',y='AI',data=concatdf,hue='pre/post',showfliers=False,palette='hls')
    plt.suptitle('<'+filenamelist[i]+': pre vs. post matched AI>', y=1.03, size=11)
    plt.show()
