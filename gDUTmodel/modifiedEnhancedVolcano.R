...
modifiedEnhancedVolcano <- function(
    toptable,
    lab,
    x,
    y,
    ...
    raster = FALSE)
{
  ...
  toptable$Sig <- 'NS'
  toptable$Sig[(abs(toptable[[x]]) > FCcutoff) & (abs(toptable[[y]]) > FCcutoff)] <- 'FC'
  
  toptable$Sig[(abs(toptable[[x]]) < FCcutoff) & (abs(toptable[[y]]) > FCcutoff)] <- 'P'
  
  toptable$Sig[(abs(toptable[[x]]) > FCcutoff) & (abs(toptable[[y]]) < FCcutoff)] <- 'FC_P'
  ...
  plot <- plot +
    ...
    geom_hline(yintercept = c(-pCutoff, pCutoff),
               linetype = cutoffLineType,
               colour = cutoffLineCol,
               size = cutoffLineWidth)
