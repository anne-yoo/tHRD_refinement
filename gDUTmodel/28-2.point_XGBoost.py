...
forfeature = pd.read_csv('/home/jiye/jiye/copycomparison/gDUTresearch/202311_analysis/featureselection/pointbiserial_143.txt', sep='\t', index_col=0)
...
for tpr in interpolated_tprs:
    plt.plot(mean_fpr, tpr, color='darkgreen', alpha=0.3)  # Adjust alpha for opacity
...
for recall, precision in pr_curves:
    # Interpolate precision values to the common recall values
    # Ensure precision is 1 at recall 0 by appending to the beginning
    recall = np.append([0], recall)
    precision = np.append([1], precision)
    
    # Interpolate and append the result to the list
    interpolated_precision = np.interp(mean_recall, recall[::-1], precision[::-1])
    interpolated_precisions.append(interpolated_precision)

# Calculate the mean precision for each recall value
...
for interpolated_precision in interpolated_precisions:
    plt.plot(mean_recall, interpolated_precision, color='darkgreen', alpha=0.3)
