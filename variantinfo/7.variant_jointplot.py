...
for i in range(4):
    ...
    for j in range(3):
        kind = filekind[j]
        condition1 = (0.4<=kind['genome_AF']) & (kind['genome_AF'] <= 0.6)
        kind = kind.loc[condition1,:]
        ...
        plt.savefig('/home/omics/DATA6/jiye/copycomparison/variantinfo/modifiedAF/modifiedfigures/jointplot/germline-'+filenamelist[i]+'_'+forsaving[j]+'.png', bbox_inches='tight', dpi=150)
        plt.show()
...
for i in range(4):
    ...
    for j in range(3):
        ...
        sns.set(rc = {'figure.figsize':(9,12)})
        sns.set_style('whitegrid')
        g = sns.jointplot(data=kind, x='genome_AF', y='RNA_AF', hue="response", palette="hls", xlim=(-0.1,1.1), ylim=(-0.1,1.1))
        plt.suptitle('< ' + filenamelist[i] + ': ' + filekindname[j]+ '>',y=1.02, fontsize=12)
        plt.tight_layout()
        x0, x1 = g.ax_joint.get_xlim()
        y0, y1 = g.ax_joint.get_ylim()
        lims = [max(x0, y0), min(x1, y1)]
        g.ax_joint.plot(lims, lims, '--g')
        plt.savefig('/home/omics/DATA6/jiye/copycomparison/variantinfo/withsalvage/depth5/jointplot_depth5/'+filenamelist[i]+'_'+forsaving[j]+'.png', bbox_inches='tight', dpi=150)
        plt.show()
