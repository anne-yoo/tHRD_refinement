...
for i in range(4):
    ...
    sns.kdeplot(ax=axes[0], data=res, x='deltaAF',hue_order=orders, hue="impact", palette='husl', fill=True, common_norm=False, alpha=0.4)
    sns.kdeplot(ax=axes[1], data=non_res, x='deltaAF', hue_order=orders, hue="impact", palette='husl', fill=True, common_norm=False, alpha=0.4)
    axes[0].set_title("Responder")
    axes[1].set_title("Non-Responder")
    plt.savefig('/home/omics/DATA6/jiye/copycomparison/variantinfo/withsalvage/depth10/density_depth10/'+filenamelist[i]+'.png', bbox_inches='tight', dpi=150)
    plt.show()
...
for i in range(4):
    ...
    axes[0].set_title("Responder")
    axes[1].set_title("Non-Responder")
    plt.savefig('/home/omics/DATA6/jiye/copycomparison/variantinfo/withsalvage/depth5/density_depth5/'+filenamelist[i]+'.png', bbox_inches='tight', dpi=150)
    plt.show()
