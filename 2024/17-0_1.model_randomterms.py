...
import random
random_terms = random.sample(terms, 800)
##################################################
##go_term_ids = [term[0] for term in random_terms]
go_term_ids = [term[0] for term in terms]
##################################################
...
for term in go_term_ids:
    ...
    if input.shape[0] > 0:
        ...
        X = input.T
        y = y.loc[responder, :].values.ravel()
        ...
        result_file_path = '/home/jiye/jiye/copycomparison/gDUTresearch/202403_analysis/model/model1_random/result_fixedparams/' + term + '_result_summary.txt'
